...
function slideAnime() {
  ...
  $(".leftAnime").each(function() {
    ...
    if (scroll >= elemPos - windowHeight) {
      ...
      $(this)
        ...
        .addClass("slideAnimeRightLeft") //子要素は親要素のアニメーションに影響されないように逆の指定をし元の位置をキープするアニメーションをおこなう
    } else {
      //左から右へ表示するクラスを取り除く
  ...
  $(".rightAnime").each(function() {
    ...
    if (scroll >= elemPos - windowHeight) {
      ...
      $(this)
        .children(".rightAnimeInner")
        .addClass("slideAnimeLeftRight") //子要素は親要素のアニメーションに影響されないように逆の指定をし元の位置をキープするアニメーションをおこなう
    } else {
      //右から左へ表示するクラスを取り除く
...
$(window).on("load", function() {
  $("#splash-logo")
    .delay(1200)
    .fadeOut("slow") //ロゴを1.2秒でフェードアウトする記述
